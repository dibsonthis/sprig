...
const _setMetaProp = exec(`(obj, key, value) => {
    if (obj.meta) {
        obj.meta[key] = value
    } else {
        obj.meta = {
...
const setMetaProp = (obj, key, value) => {
    raw(obj)->_setMetaProp(key, value)
    return obj
}

const _getMetaProp = exec(`(obj, key) => obj.meta?.[key]`)

const getMetaProp = (obj, key) => {
    return raw(obj)->_getMetaProp(key)
}
